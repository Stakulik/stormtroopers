class SwUnits::ShowSerializer < SwUnitSerializer
  ...
  def define_additional_params(object)
    if object.class.to_s == "Person"
      %w(id name birth_year eye_color gender hair_color height mass skin_color)
    elsif object.class.to_s == "Planet"
      %w(id name rotation_period orbital_period climate diameter gravity surface_water terrain
        population)
    else
      %w(id name model manufacturer cost_in_credits max_atmosphering_speed passengers
      cargo_capacity consumables hyperdrive_rating MGLT starship_class length crew)
    end
  end
